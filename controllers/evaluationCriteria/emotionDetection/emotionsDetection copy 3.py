...
class EmotionsDetection(DetectionBehavior):
    ...
    def my_custom_sink(self, predictions: dict, video_frame: VideoFrame, camera_ID):  # Added cameraID parameter
        # get the text labels for each prediction
        labels = [p["class"] for p in predictions["predictions"]]
        amount = len(labels)  # Assuming amount is the count of detected emotions
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print({"cameraID": camera_ID, "emotion": labels, "amount": amount, "time": current_time})
    ...
    def detect(self, frames, camera_ID, camera_url):
        ...
        pipeline = InferencePipeline.init(
            ...
            on_prediction=lambda predictions, video_frame: self.my_custom_sink(predictions, video_frame, camera_ID),  # Fixed method reference
        )
        pipeline.start()
        pipeline.join()
